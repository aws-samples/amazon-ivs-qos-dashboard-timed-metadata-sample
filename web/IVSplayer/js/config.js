export const config = {
  // live channel's playback URL, 893648527354.channel.xhP3ExfcX8ON.m3u8 is a permanent test channel with quiz timed metadata
  "PlaybackURL": "https://fcc3ddae59ed.us-west-2.playback.live-video.net/api/video/v1/us-west-2.893648527354.channel.xhP3ExfcX8ON.m3u8",
  // end point for sending player QoS (i.e., playback summary) events
  "SendQoSEventURL": "",
  // end point for sending timed metadata feedback (e.g., quiz answer) events 
  "SendQuizAnswerURL":"",
  // ask Jai
  "DeliveryStreamName":"DELIVERY_STREAM_NAME",
};
